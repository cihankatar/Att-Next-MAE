...
from data.data_loader_mae_2 import loader
...
##### MASK WEIGHTED LOSS ###
def mw_mse_loss(reconstructed, images, mask_map):
...
def main():
    ...
    def run_epoch(loader,epoch_idx,training=True):
        ...
        with torch.set_grad_enabled(training):
            for batch_idx, images in enumerate(loader):  #tqdm(loader, desc="Training" if training else "Validating", leave=False):
                ...
                masked_images, mask_map = Cutout(images, pr=1, pad_size=10, mask_ratio=0.5)
                ...
                loss             = mw_mse_loss(out, images, mask_map)
                ...
                if log_images:
                    ...
                    img_grid = [
                        ...
                        #wandb.Image(to_edge_img(original_img),  caption="Input Gabor Edges"),
                        wandb.Image(to_edge_img(out),           caption="Output Gabor Edges")
                    ]

                    wandb.log({f"Reconstruction Epoch {epoch_idx}": img_grid})
                
                # #### SAVING PDF TO USE PRESENTATION
                # if epoch_idx >= 190:  
                #     save_dir = os.path.join("outputs", f"epoch_{epoch_idx}")
                #     os.makedirs(save_dir, exist_ok=True)
                #     titles = ["Original","Clean Input","Masked Input","Reconstructed Image","Inpput Gabor Edge","Output Gabor Edge"]
                #     images = [to_img(original_img),to_img(clean_image),to_img(masked_img),to_img(reconstructed_img),to_edge_img(clean_images),to_edge_img(out)]

                #     pdf_path = os.path.join(save_dir, f"reconstruction_epoch_{epoch_idx}.pdf")
    ...
    for epoch in trange(config['epochs'], desc="Epochs"):
        ...
        # Training
        train_loss = run_epoch(train_loader, epoch_idx,training=True )
