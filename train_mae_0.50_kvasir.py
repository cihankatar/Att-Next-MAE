...
def main():
    ...
    data, training_mode, op = 'kvasir_1', "ssl", "train"
    best_loss = float("inf")
    device      = using_device()
    ...
    def run_epoch(loader,epoch_idx,training=True):
        ...
        with torch.set_grad_enabled(training):
            for batch_idx, images in enumerate(loader):  #tqdm(loader, desc="Training" if training else "Validating", leave=False):
                ...
                masked_images, mask_map = Cutout(images, pr=1, pad_size=10, mask_ratio=0.50)
                ...
                seg_loss             = F.binary_cross_entropy_with_logits(out, pseudo_mask)
                seg_loss             = w*seg_loss
                ...
                if not training:
                    with torch.no_grad():
                        ...
                        target_img = rgb_to_grayscale(target_img)
